...
var
  ...
  MessageObj, TextObj: TJSONObject;
  InputStream, ResponseStream: TStringStream;
  ContentData, ItemData: TJSONData;
  i: Integer;
begin
  Result := '';
  ...
  try
    ...
    if Context <> '' then
      MessageObj.Add('content',
        Format('Here is the code context:%s%s%s%sQuestion: %s',
    ...
    try
      ...
      // --- Correct TFPHTTPClient.Post usage ---
      FHTTPClient.Post(
    ...
    try
      ...
      begin
        ...
        begin
          ItemData := ContentArray.Items[i];
          if (ItemData.JSONType = jtObject) then
          begin
            TextObj := TJSONObject(ItemData);

...
var
  FileName: string;
  PSEngine: TPascalScriptEngine;
  UseSimpleInterpreter: Boolean;

begin
  if ParamCount < 1 then
  begin
    Writeln('Usage: CTMInterpreter <filename> [--simple]');
    Writeln('');
    Writeln('Supported file types:');
    Writeln('  .ctm   - Simple CTM language (built-in)');
    Writeln('  .kayte - Simple Kayte language (built-in)');
    Writeln('  .pas   - Pascal Script (requires PascalScript library)');
    Writeln('');
    Writeln('Options:');
    Writeln('  --simple  Force simple interpreter mode');
    Writeln('');
    Writeln('Note: Full Pascal script support requires the PascalScript library');
    Writeln('Install: https://github.com/remobjects/pascalscript');
    Halt(1);
  end;
  ...
  UseSimpleInterpreter := (ParamCount > 1) and (ParamStr(2) = '--simple');

  if not UseSimpleInterpreter then
    UseSimpleInterpreter := (Pos('.ctm', LowerCase(FileName)) > 0) or
                            (Pos('.kayte', LowerCase(FileName)) > 0);

  try
    if UseSimpleInterpreter then
    begin
      // Use the simple built-in interpreter
      Writeln('Using simple built-in interpreter...');
      Writeln('');
    ...
    begin
      // Attempt to use Pascal Script for .pas files
      Writeln('Attempting to use Pascal Script engine...');
      Writeln('');
      PSEngine := TPascalScriptEngine.Create;
      try
        if not PSEngine.LoadAndExecute(FileName) then
        begin
          Writeln('');
          Writeln('Falling back to simple interpreter...');
          Writeln('To use .pas files, consider renaming to .ctm or .kayte');
          Writeln('and using simplified syntax.');
        end;
      finally
        PSEngine.Free;
      end;

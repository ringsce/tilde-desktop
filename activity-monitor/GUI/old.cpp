    ...
    QStringList getProcessList() {
        ...
        QStringList SystemMonitor::getProcessList() {
            ...
            for (int i = 0; i < count; ++i) {
                ...
                double memMB = ti.pti_resident_size / (1024.0 * 1024.0);
            ...
            std::sort(rows.begin(), rows.end(),
                      [](const Row &a, const Row &b) { return a.cpu > b.cpu; });

            out << "PID      USER       CPU%     MEMORY     THR  PPID   PROCESS";
            out << QString("─").repeated(90);

            for (int i = 0; i < qMin(150, rows.size()); ++i) {
                const auto &r = rows[i];

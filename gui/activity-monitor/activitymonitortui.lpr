...
begin
  ...
  try
    ...
    try
      AProcess.Execute;
      TempLines.LoadFromStream(AProcess.Output);

      Count := 0;
      // Filter and add to Lines
      ...
      on E: Exception do
        Lines.Add('Error getting process list: ' + E.Message);
    end;
  finally
    AProcess.Free;
    TempLines.Free;
...
begin
  ...
  // CPU Usage Box
  DrawBox(2, 5, 96, 6, 'CPU Usage');
  GotoXY(4, 7);
  SetColor(7, 0);
  Write('Current CPU Usage: ');
  if CPUUsage > 75 then
    SetColor(1, 0)
  else if CPUUsage > 50 then
    SetColor(3, 0)
  else
    SetColor(2, 0);
  Write(Format('%3d%%', [CPUUsage]));
  ResetColor;

  GotoXY(4, 8);
  Write('Progress: ');
  DrawProgressBar(14, 8, 80, CPUUsage, False);

  GotoXY(4, 9);
  SetColor(6, 0);
  Write('CPU cores are ', CPUUsage, '% utilized');
  ResetColor;

  // Memory Usage Box
  DrawBox(2, 11, 96, 6, 'Memory Usage');
  GotoXY(4, 13);
  SetColor(7, 0);
  Write('Current Memory Usage: ');
  if MemUsage > 75 then
    SetColor(1, 0)
  else if MemUsage > 50 then
    SetColor(3, 0)
  else
    SetColor(2, 0);
  Write(Format('%3d%%', [MemUsage]));
  ResetColor;

  GotoXY(4, 14);
  Write('Progress: ');
  DrawProgressBar(14, 14, 80, MemUsage, False);

  GotoXY(4, 15);
  SetColor(6, 0);
  Write('RAM Usage: ', MemStats);
  ResetColor;

  // CPU History Graph
  DrawBox(2, 17, 96, 14, 'CPU History Graph (last 80 readings)');
  DrawCPUGraph(4, 19, 92, 10);

  // Add percentage labels on the graph
  GotoXY(3, 19);
  SetColor(7, 0);
  Write('100%');
  GotoXY(3, 24);
  ...
  GotoXY(2, 32);
  SetColor(0, 7);
  Write(' [1] Dashboard  [2] Processes  [Q] Quit  [ENTER] Refresh ');
  ResetColor;

  GotoXY(2, 34);
  SetColor(3, 0);
  Write('Tip: Just press ENTER to refresh the display');
  ResetColor;
...
begin
  ...
  try
    ...
    GotoXY(4, 5);
    WriteLn('Total processes: ', Lines.Count);
    WriteLn;
    ...
    begin
      GotoXY(4, 7 + MaxLines);
      SetColor(3, 0); // Yellow
      Write('... and ', Lines.Count - MaxLines, ' more processes (showing first ', MaxLines, ')');
      ResetColor;
    end;
    ...
    begin
      GotoXY(4, 8);
      SetColor(1, 0); // Red
      Write('No processes found or error reading process list');
      ResetColor;
    end;
...
begin
  ...
  repeat
    ...
    GotoXY(2, 36);
    Write('Enter choice (or press ENTER to refresh): ');
    Flush(Output);
    ReadLn(Choice);
    Choice := UpCase(Trim(Choice));

    if Choice = '' then
      Choice := 'R'; // Empty input = refresh
  ...
  until (Choice = 'Q') or (Choice = 'QUIT') or (Choice = 'EXIT');
